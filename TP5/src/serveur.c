/* 16/10/2024 Fichier : serveur.c
*Communication avec le fichier client.c pour afficher une chaine de caractère ou le résultat d'une opération
 ...
 * @return EXIT_SUCCESS en cas de succès, EXIT_FAILURE en cas d'erreur.
 */
  void recois_envoie_message(int client_socket, char *data) {
  char message[1000];
  printf("Saisissez un message à envoyer au client : ");
...
//Si le message commence par le mot  "calcule:"  on renvoie la valeur de l'opération
  if (strcmp(code, "calcule:") == 0) {
    float res;
    res= recois_numeros_calcule(data);
    sprintf(data, "calcule: %f ",res); //concaténation de "calcule:" et du résultat
...
int recois_numeros_calcule(char *data){
  char * op = malloc(sizeof(op));  // On laisse de la mémoire dynamique pour l'opérateur
  float num1;  
  float num2;  
